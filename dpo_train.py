...
def preprocess(
    ...
    chosen_conversations = sample["conversations"][1]["chosen"]+ conv.sep2
    #rejected_sources = sample["rejected"]
    #rejected_conversations = rejected_sources[1]["role"] + ": " +  rejected_sources[1]["content"] + conv.sep2
    rejected_conversations = sample["conversations"][1]["reject"]+ conv.sep2
    return dict(
        prompt=prompt,
...
def train():
    ...
    trainer = DPOTrainer(
        ...
        max_prompt_length = 512, max_length = training_args.model_max_length,
    )
